...
const TimelineItem: React.FC<TimelineItemProps> = ({ time, title, description, isLeft }) => (
  <div className={`mb-8 flex justify-between items-center w-full ${isLeft ? 'flex-row-reverse' : 'flex-row'}`}>
    ...
    <div className="order-1 bg-gray-100 rounded-lg shadow-xl w-5/12 px-6 py-4">
      ...
      <time className="mb-3 font-normal leading-none text-gray-500 text-sm">{time}</time>
      <p className="text-sm font-medium leading-snug tracking-wide text-gray-600 text-opacity-100">{description}</p>
    </div>
  </div>
);
